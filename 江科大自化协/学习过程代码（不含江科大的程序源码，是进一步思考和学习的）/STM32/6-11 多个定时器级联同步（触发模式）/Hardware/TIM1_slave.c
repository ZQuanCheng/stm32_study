...
void TIM1_PWM_Init(void)
{
	...
	/*配置时钟源*/
	TIM_InternalClockConfig(TIM1);		//选择TIM3为内部时钟，若不调用此函数，TIM默认也为内部时钟
	
	/*时基单元初始化*/
	// 目标输出：一个频率10kHz、占空比50%，分辨率为1%的PWM波形。对应的 ARR=100-1=99、PSC=72-1=71、CCR=50。
	TIM_TimeBaseInitTypeDef TIM_TimeBaseInitStructure;				//定义结构体变量
	TIM_TimeBaseInitStructure.TIM_ClockDivision = TIM_CKD_DIV1;     //时钟分频，选择不分频，此参数用于配置滤波器时钟，不影响时基单元功能
	// 定时频率Freq = 计数器的溢出频率CK_CNT_OV = CK_PSC / (PSC + 1) / (ARR + 1) = 72MHz / (72-1+1) / (100-1+1) = 10kHz。
