...
int main(void)
{
	...
	{	
		...
		AD_Value = ADC_GetConversionValue(ADC2);    //读取ADC2_DR规则数据寄存器中的转换结果
		
		OLED_ShowNum(1, 8, ADC_WATCHDOG_HT, 4);		//显示ADC模拟看门狗的上限，全局变量ADC_WATCHDOG_HT在AD.c中定义
		OLED_ShowNum(2, 8, AD_Value, 4);		    //显示转换结果
...
void ADC1_2_IRQHandler(void)
{
	...
	*/
	if (ADC_GetITStatus(ADC2, ADC_IT_AWD) == SET)          //判断是否是ADC1中的AWD（转换结果超出模拟看门狗的上下限）触发的中断
	{
		ADC_ClearITPendingBit(ADC2, ADC_IT_AWD);			//清除ADC1中的AWD模拟看门狗监测到超限事件的中断标志位
															//中断标志位必须清除
