...
void TIM1_PWM_Init(void)
{
	...
	/*时基单元初始化*/
	// 目标输出：一个频率1kHz、占空比50%，分辨率为1%的PWM波形。对应的 ARR=100-1=99、PSC=36-1=35、CCR=75。
	TIM_TimeBaseInitTypeDef TIM_TimeBaseInitStructure;				            //定义结构体变量
	TIM_TimeBaseInitStructure.TIM_ClockDivision = TIM_CKD_DIV1;                 //时钟分频，选择不分频，此参数用于配置滤波器时钟，不影响时基单元功能
	// 定时频率Freq = 计数器的溢出频率CK_CNT_OV = CK_PSC / (PSC + 1) / ARR / 2 = 72MHz / (36-1+1) / 100 / 2 = 10kHz。
	TIM_TimeBaseInitStructure.TIM_CounterMode = TIM_CounterMode_CenterAligned3; //计数器模式，选择中央对齐模式3
	TIM_TimeBaseInitStructure.TIM_Prescaler = 36 - 1;				            //预分频器，即PSC的值
	TIM_TimeBaseInitStructure.TIM_Period = 100;					                //计数周期，即ARR的值
