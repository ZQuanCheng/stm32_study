...
void IC_Init(void)
{
	...
	TIM_PWMIConfig(TIM1, &TIM_ICInitStructure);						//将结构体变量交给TIM_PWMIConfig，配置TIM3的输入捕获通道
    ...
    */
	...
	/*选择触发源及从模式*/
	TIM_SelectInputTrigger(TIM1, TIM_TS_TI1FP1);					//触发源选择TI1FP1
	TIM_SelectSlaveMode(TIM1, TIM_SlaveMode_Reset);					//从模式选择复位
																	//即TI1产生上升沿时，会触发CNT归零
...
uint32_t IC_GetFreq(void)
{
	...
	return 1000000 / (TIM_GetCapture1(TIM1) + 1);		//测周法得到频率fx = fc / N，这里不执行+1的操作也可
}
...
{
	return (TIM_GetCapture2(TIM1) + 1) * 100 / (TIM_GetCapture1(TIM1) + 1);	//占空比Duty = CCR2 / CCR1 * 100，这里不执行+1的操作也可	
