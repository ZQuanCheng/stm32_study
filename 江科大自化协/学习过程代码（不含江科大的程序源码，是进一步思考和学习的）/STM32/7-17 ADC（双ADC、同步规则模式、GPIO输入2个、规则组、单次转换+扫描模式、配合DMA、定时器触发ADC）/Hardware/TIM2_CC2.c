...
void TIM2_CC2_Init(void)
{
	/*
	   ...
	   因此，这里我们没有配置PA1
	   最重要的是，PA1用于ADC12_IN1的模拟输入了，不能再用于TIM2_CH2的输出PWM	
	*/
	...
	/*配置时钟源*/
	TIM_InternalClockConfig(TIM2);		//选择TIM2为内部时钟，若不调用此函数，TIM2默认也为内部时钟
	...
	// 配置通道1
	TIM_OCInitStructure.TIM_Pulse = 5000;				// CCR寄存器值。占空比duty = CCR / (ARR+1) = 5000 / (10000-1+1) = 50%
	TIM_OC2Init(TIM2, &TIM_OCInitStructure);			// 将结构体变量交给TIM_OC2Init，配置TIM2的输出比较通道CC2
	
	
	/*输出比较：CCR预装功能*/
	// 用于配置CCR捕获/比较寄存器的预装功能的，即影子寄存器。写入的值不会立即生效，而是在更新事件才会生效，一般不用。
	// GD32中对应的是CHxAVL寄存器，代码为timer_channel_output_shadow_config(TIMER1, TIMER_CH_1, TIMER_OC_SHADOW_DISABLE);
	// STM32中是下列代码
	TIM_OC2PreloadConfig(TIM2, TIM_OCPreload_Disable);  // 使能或失能TIM2在CC2上的捕获/比较寄存器CCR2
	                                                    // 仅用于输出比较
	...
	*/
	...
	// GD32中对应的是ACR寄存器，代码为timer_auto_reload_shadow_enable(TIMER1);
	// STM32中是下列代码
