...
void EXTI_11_Init(void)
{
	...
	GPIO_Init(GPIOA, &GPIO_InitStructure);						//将PA11引脚初始化为下拉输入
	// 如果对应的GPIO配置为IPD下拉模式，外部中断线就配置为上升沿触发EXTI_Trigger_Rising
	// 如果对应的GPIO配置为IPU上拉模式，外部中断线就配置为下降沿触发EXTI_Trigger_Falling	
	
	
	/*AFIO选择中断引脚*/
	GPIO_EXTILineConfig(GPIO_PortSourceGPIOA, GPIO_PinSource11);//将外部中断的11号线映射到GPIOA，即选择PA11为外部中断引脚
	...
	// 注意，不是中断模式，这里EXTI_11仅作为事件，来触发ADC
	// 如果对应的GPIO配置为IPD下拉模式，外部中断线就配置为上升沿触发EXTI_Trigger_Rising
	// 如果对应的GPIO配置为IPU上拉模式，外部中断线就配置为下降沿触发EXTI_Trigger_Falling		
	...
	EXTI_ClearFlag(EXTI_Line11);              // 清除外部中断11号线的标志位	
}
